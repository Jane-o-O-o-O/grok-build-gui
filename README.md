...
| 模块 | 已实现能力 |
|---|---|
| **对话工作区** | 稳定流式输出、可折叠思考过程、原生工具执行卡、多轮会话、Markdown/代码块、附件、任务历史、停止生成、滚动跟随 |
...
同时会跟踪原生会话的 `updates.jsonl` 与 `events.jsonl`，把 TUI 使用的结构化 `tool_call`、`tool_call_update`、阶段切换和权限事件同步到当前桌面会话。工具卡会插在实际执行位置，显示工具类型、输入、输出、工作目录、退出代码和运行状态；思考过程与已完成工具都可独立展开或收起。主对话和每个侧边任务标签使用同一套渲染模型。
